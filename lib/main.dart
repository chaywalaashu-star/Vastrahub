// ╔══════════════════════════════════════════════════════════════╗
// ║    KIRANA STORE MANAGEMENT SYSTEM — UI + BACKEND CONNECTED       ║
...
class GradientAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  @override Pack Size get preferredSize => const Pack Size.fromHeight(60);
  @override
    ...
    body: Container(decoration: const BoxDecoration(gradient: AppColors.gradientRB),
      child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ScaleTransition(scale: _s, child: Container(width: 110, height: 110,
          ...
          child: const Icon(Icons.shopping_basket_rounded, size: 60, color: AppColors.red))),
        const SizedBox(height: 28),
        const Text('KIRANA STORE MANAGER', style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.w900, letterSpacing: 3)),
    ...
    body: Container(height: double.infinity, decoration: const BoxDecoration(gradient: AppColors.gradientRB),
      child: SingleChildScrollView(child: Column(children: [
        ...
        Container(width: 86, height: 86, decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle,
          ...
          child: const Icon(Icons.shopping_basket_rounded, size: 48, color: AppColors.red)),
        const SizedBox(height: 14),
        const Text('KIRANA STORE MANAGER', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: 2.5)),
        const SizedBox(height: 28),
...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  Future<void> _load() async {
    ...
    final lowStockBlocks = await InventoryService.instance.getLowStockSubUnits();
    if (!mounted) return;
    setState(() { _items = inv.length; _revenue = rev; _workers = wrks.where((w)=>w.isActive).length; _returns = lowStockBlocks.length; _loading = false; });
  }
          ...
          SliverToBoxAdapter(child: _loading ? const Padding(padding: EdgeInsets.only(top:60), child: Center(child: CircularProgressIndicator(color: AppColors.red)))
            : Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 3, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 0.88, children: [
                ...
                _nCard('Udhari', Icons.account_balance_wallet_rounded, const Color(0xFF00838F), route: '/udhari'),
              ]),
            ]))),
        ])));
  }
...
class _InventorySetupScreenState extends State<InventorySetupScreen> with SingleTickerProviderStateMixin {
  ...
  final _uPackSizes = TextEditingController();
  String _uCategory = 'Grocery';
  List<InventoryUnit> _units = [];
  Map<int, List<InventorySubUnit>> _subUnitsMap = {};
  bool _savingUnit = false;

  // Item fields — CRN/Naam optional hain, sirf yaad rakhne ke liye.
  // Unit + Pack Size + Quantity hi asal mein zaroori hain — quantity yahi se
  // block (sub-unit) ke total mein feed hoti hai.
  ...
  final _categories = ['Grocery', 'Dairy', 'Snacks', 'Beverages', 'Household', 'Personal Care', 'Atta/Rice/Dal', 'Masale'];

  @override void initState() { super.initState(); _tab = TabController(length: 2, vsync: this); _loadUnits(); }
  @override void dispose() { _tab.dispose(); _uName.dispose(); _uPackSizes.dispose(); _iName.dispose(); _iCrn.dispose(); _iPrice.dispose(); _iQty.dispose(); super.dispose(); }

  Future<void> _loadUnits() async {
    final u = await InventoryService.instance.getAllUnits();
    final subMap = await InventoryService.instance.getSubUnitsGroupedByUnit();
    if (!mounted) return;
    setState(() {
      _units = u;
      _subUnitsMap = subMap;
      if (u.isNotEmpty && (_selUnitId == null || !u.any((x) => x.id == _selUnitId))) _selUnitId = u.first.id;
      _syncSelectedSubUnit();
    });
  }

  void _syncSelectedSubUnit() {
    final unit = _units.where((u) => u.id == _selUnitId).toList();
    final subs = unit.isEmpty ? <InventorySubUnit>[] : (_subUnitsMap[unit.first.id] ?? []);
    if (subs.isEmpty) {
      _selSubUnit = '';
    } else if (!subs.any((s) => s.packSize == _selSubUnit)) {
      _selSubUnit = subs.first.packSize;
    }
  }

  // Unit banao — sirf DHAANCHA (structure) banta hai, koi stock quantity
  // yahan nahi daalte. Agar naam+category pehle se hai to REPLACE nahi hota,
  // balki diya gaya pack size us existing unit ke andar nayi SUB-UNIT ban kar
  // jud jaata hai (stock baad mein "Item Add Karo" tab se aati hai).
  Future<void> _saveUnit() async {
    if (_uName.text.trim().isEmpty) { showSnack(context, 'Unit naam daalo!', isError: true); return; }
    if (_uPackSizes.text.trim().isEmpty) { showSnack(context, 'Pack size daalo! (jaise: 500g ya 1kg)', isError: true); return; }
    setState(() => _savingUnit = true);
    final r = await InventoryService.instance.addOrUpdateUnit(
      name: _uName.text.trim(), category: _uCategory,
      packSizesInput: _uPackSizes.text.trim());
    if (!mounted) return;
    setState(() => _savingUnit = false);
    if (r.isSuccess) {
      showSnack(context, 'Saved! "${_uName.text.trim()}" ($_uCategory) mein pack size "${_uPackSizes.text.trim()}" ban gayi ✓');
      // Unit naam jaan bujh kar clear nahi karte — taaki isi unit mein
      // agla pack size turant add kar sako. Sirf pack size field clear hoga.
      _uPackSizes.clear();
      _loadUnits();
    } else {
  ...
  Future<void> _editThreshold(InventorySubUnit su) async {
    final ctrl = TextEditingController(text: '${su.lowStockThreshold}');
    final result = await showDialog<int>(context: context, builder: (dCtx) => AlertDialog(
      title: Text('Low-Stock Alert — Pack Size "${su.packSize}"'),
      content: TextField(controller: ctrl, keyboardType: TextInputType.number, autofocus: true,
        decoration: const InputDecoration(labelText: 'Kitni quantity pe alert dikhe?')),
      actions: [
        TextButton(onPressed: () => Navigator.pop(dCtx), child: const Text('Cancel')),
        ElevatedButton(onPressed: () => Navigator.pop(dCtx, int.tryParse(ctrl.text) ?? su.lowStockThreshold),
          child: const Text('Save')),
      ]));
    if (result != null) {
      await InventoryService.instance.updateSubUnitThreshold(su.id!, result);
      if (mounted) { showSnack(context, 'Threshold update ho gaya ✓'); _loadUnits(); }
    }
  }

  Future<void> _saveItem() async {
    if (_units.isEmpty) { showSnack(context, 'Pehle unit banao!', isError: true); return; }
    if (_selSubUnit.isEmpty) { showSnack(context, 'Pehle is item mein ek pack size (sub-unit) banao!', isError: true); return; }
    final qty = int.tryParse(_iQty.text) ?? 0;
    if (qty <= 0) { showSnack(context, 'Sahi quantity daalo!', isError: true); return; }
    setState(() => _saving = true);
    final unit = _units.firstWhere((u) => u.id == _selUnitId, orElse: () => _units.first);
    final r = await InventoryService.instance.addItem(InventoryItem(
      crn: _iCrn.text.trim().toUpperCase(), name: _iName.text.trim(),
      category: unit.category, packSize: _selSubUnit,
    ...
    setState(() => _saving = false);
    if (r.isSuccess) {
      showSnack(context, '$qty add ho gaye "${unit.name} - $_selSubUnit" mein ✓');
      _iName.clear(); _iCrn.clear(); _iPrice.clear(); _iQty.text = '1';
      _loadUnits();
    } else {
      showSnack(context, r.error!, isError: true);
    }
  ...
  Widget _unitTab() => ListView(padding: const EdgeInsets.all(16), children: [
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 16),
      TextField(controller: _uName, decoration: const InputDecoration(labelText: 'Item naam (jaise: Namak, Biscuit)', prefixIcon: Icon(Icons.category_rounded, color: AppColors.red))),
      const SizedBox(height: 14),
      const Text('Category', style: TextStyle(fontSize: 13, color: AppColors.textGrey, fontWeight: FontWeight.w600)),
      const SizedBox(height: 8),
      Wrap(spacing: 8, children: _categories.map((g) {
        final sel = _uCategory == g;
        return GestureDetector(onTap: () => setState(() => _uCategory = g),
      ...
      const SizedBox(height: 14),
      TextField(controller: _uPackSizes, decoration: const InputDecoration(labelText: 'Pack Size (jaise: 500g ya 1kg) — comma se ek se zyada bhi', hintText: '500g  ya  250g,500g,1kg  ya  1 packet', prefixIcon: Icon(Icons.straighten_rounded, color: AppColors.red))),
      const SizedBox(height: 16),
      SizedBox(width: double.infinity, child: ElevatedButton.icon(
        onPressed: _savingUnit ? null : _saveUnit,
        icon: _savingUnit ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.save_rounded),
        label: const Text('Unit / Sub-Unit Save Karo'))),
    ])),
    const SizedBox(height: 16),
      ...
      return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            chip(u.category, AppColors.blue),
          ]),
          const SizedBox(height: 10),
          subs.isEmpty
            ? Text('Koi sub-unit (pack size) nahi hai abhi.', style: TextStyle(fontSize: 12, color: Colors.grey.shade400))
            : Wrap(spacing: 6, runSpacing: 6, children: subs.map((s) => GestureDetector(
                onTap: () => _editThreshold(s),
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(
                    color: s.isOutOfStock ? AppColors.red.withValues(alpha: 0.08) : (s.isLowStock ? AppColors.warning.withValues(alpha: 0.1) : AppColors.bgLight),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: s.isOutOfStock ? AppColors.red : (s.isLowStock ? AppColors.warning : Colors.grey.shade300))),
                  child: Text('${s.packSize} • ${s.totalQuantity} pcs',
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
                      color: s.isOutOfStock ? AppColors.red : (s.isLowStock ? AppColors.warning : AppColors.textDark)))))).toList()),
          if (subs.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 6),
            child: Text('Pack Size pe tap karke low-stock alert limit set karo', style: TextStyle(fontSize: 10, color: Colors.grey.shade400))),
        ]));
    }),
  ]);
  ...
  Widget _itemTab() => ListView(padding: const EdgeInsets.all(16), children: [
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const SectionTitle(text: 'Stock Add Karo'),
      const SizedBox(height: 6),
      Text('Unit + Pack Size + Quantity daalo — barcode/naam optional hai.', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
      const SizedBox(height: 16),
      if (_units.isEmpty)
        ...
        DropdownButtonFormField<int>(
          value: (_selUnitId != null && _units.any((u) => u.id == _selUnitId)) ? _selUnitId : _units.first.id,
          decoration: const InputDecoration(labelText: 'Unit Select Karo', prefixIcon: Icon(Icons.category_rounded, color: AppColors.blue)),
          items: _units.map((u) => DropdownMenuItem(value: u.id, child: Text('${u.name} (${u.category})'))).toList(),
          onChanged: (v) => setState(() { _selUnitId = v!; _syncSelectedSubUnit(); })),
      const SizedBox(height: 12),
      Builder(builder: (_) {
        final unit = _units.where((u) => u.id == _selUnitId).toList();
        final subs = unit.isEmpty ? <InventorySubUnit>[] : (_subUnitsMap[unit.first.id] ?? []);
        ...
        if (subs.isEmpty) {
          return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.warning.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)),
            child: const Text('Is item mein abhi koi pack size (sub-unit) nahi hai. Pehle "Unit Banao" tab se pack size add karo.', style: TextStyle(color: AppColors.warning, fontSize: 12)));
        }
        return DropdownButtonFormField<String>(
          value: subs.any((s) => s.packSize == _selSubUnit) ? _selSubUnit : subs.first.packSize,
          decoration: const InputDecoration(labelText: 'Sub-Unit (Pack Size) Select Karo', prefixIcon: Icon(Icons.straighten_rounded, color: AppColors.blue)),
          items: subs.map((s) => DropdownMenuItem(value: s.packSize, child: Text('${s.packSize} (abhi ${s.totalQuantity} pcs)'))).toList(),
          onChanged: (v) => setState(() => _selSubUnit = v!));
      }),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: TextField(controller: _iQty, keyboardType: TextInputType.number,
      ...
      const SizedBox(height: 6),
      Text('Optional — barcode wale items ke liye (tracking/scan ke liye)', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
      const SizedBox(height: 10),
      TextField(controller: _iName, decoration: const InputDecoration(labelText: 'Item ka naam (optional)', prefixIcon: Icon(Icons.shopping_basket_rounded, color: AppColors.blue))),
      const SizedBox(height: 12),
      TextField(controller: _iCrn, textCapitalization: TextCapitalization.characters,
        decoration: const InputDecoration(labelText: 'Barcode / QR Number (optional)', hintText: 'Barcode ho to daalo ya scan karo', prefixIcon: Icon(Icons.qr_code_rounded, color: AppColors.blue))),
      const SizedBox(height: 20),
      SizedBox(width: double.infinity, height: 50, child: ElevatedButton.icon(
        ...
        label: const Text('Stock Add Karo'))),
    ])),
    const SizedBox(height: 14),
    Row(children: [
...
class _InventoryDetailScreenState extends State<InventoryDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final subMap = await InventoryService.instance.getSubUnitsGroupedByUnit();
    if (!mounted) return;
    setState(() { _units = units; _subUnitsMap = subMap; _loading = false; });
  }
    ...
    body: _loading ? buildLoader() : _units.isEmpty
      ...
      : RefreshIndicator(onRefresh: _load, color: AppColors.red,
          child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: _units.length,
            itemBuilder: (_, i) => _UnitCard(unit: _units[i], subUnits: _subUnitsMap[_units[i].id] ?? []))));
}

class _UnitCard extends StatelessWidget {
  final InventoryUnit unit;
  final List<InventorySubUnit> subUnits;
  const _UnitCard({required this.unit, this.subUnits = const []});

  Color get _catColor {
    switch (unit.category) {
      case 'Dairy':          return AppColors.red;
      case 'Snacks':         return const Color(0xFF6A1B9A);
      case 'Beverages':      return const Color(0xFF00838F);
      case 'Household':      return const Color(0xFF424242);
      case 'Personal Care':  return const Color(0xFFAD1457);
      case 'Atta/Rice/Dal':  return const Color(0xFFEF6C00);
      case 'Masale':         return const Color(0xFFD84315);
      default:               return AppColors.blue; // Grocery
    }
  }
    ...
    child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16),
      ...
      child: Row(children: [
        Container(width: 50, height: 50,
          decoration: BoxDecoration(color: _catColor.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(12)),
          child: Icon(Icons.shopping_basket_rounded, color: _catColor, size: 26)),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            subUnits.isEmpty
              ? '${unit.category} • Koi pack size nahi'
              : '${unit.category} • Pack Sizes: ${subUnits.map((s) => s.packSize).join(", ")}',
...
class _UnitItemsScreenState extends State<UnitItemsScreen> {
  Map<InventorySubUnit, List<InventoryItem>> _grouped = {};
  bool _loading = true;
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final grouped = await InventoryService.instance.getItemsGroupedBySubUnit(widget.unit.id!);
    if (!mounted) return;
    setState(() { _grouped = grouped; _loading = false; });
  }

  Map<InventorySubUnit, List<InventoryItem>> get _filtered {
    final q = _search.text.trim().toLowerCase();
    if (q.isEmpty) return _grouped;
    final out = <InventorySubUnit, List<InventoryItem>>{};
    _grouped.forEach((su, items) {
      final matched = items.where((i) =>
        i.displayName.toLowerCase().contains(q) || i.crn.toLowerCase().contains(q) || i.brand.toLowerCase().contains(q)).toList();
      if (matched.isNotEmpty || su.packSize.toLowerCase().contains(q)) out[su] = matched;
    });
    return out;
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    return Scaffold(
      appBar: GradientAppBar(title: '${widget.unit.name} (${widget.unit.category})'),
      body: Column(children: [
        Container(color: Colors.white, padding: const EdgeInsets.all(12),
          child: TextField(controller: _search, onChanged: (_) => setState(() {}),
            decoration: InputDecoration(hintText: 'Pack Size, naam ya barcode search karo...', prefixIcon: const Icon(Icons.search_rounded, color: AppColors.blue),
              suffixIcon: _search.text.isNotEmpty ? IconButton(icon: const Icon(Icons.clear_rounded), onPressed: () { _search.clear(); setState(() {}); }) : null))),
        Expanded(child: _loading ? buildLoader() : groups.isEmpty
          ? buildEmpty('Is unit mein koi pack size (sub-unit) nahi hai.', Icons.inventory_2_rounded)
          : ListView(
              padding: const EdgeInsets.fromLTRB(16, 8, 16, 16),
              children: groups.entries.map((entry) {
                final su = entry.key;
                final items = entry.value;
                final color = su.isOutOfStock ? AppColors.red : (su.isLowStock ? AppColors.warning : AppColors.success);
                return Container(margin: const EdgeInsets.only(bottom: 16),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    // Pack Size ka header — yahi clear karta hai ye kaunsi pack size ka block hai
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                      decoration: BoxDecoration(color: color.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: color.withValues(alpha: 0.3))),
                      child: Row(children: [
                        Icon(Icons.straighten_rounded, size: 18, color: color),
                        const SizedBox(width: 8),
                        Text('Pack Size: ${su.packSize}', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: color)),
                        const Spacer(),
                        Text(su.isOutOfStock ? 'OUT OF STOCK' : '${su.totalQuantity} pcs total',
                          style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: color)),
                      ])),
                    const SizedBox(height: 8),
                    if (items.isEmpty)
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                        child: Text('Koi tracked entry nahi hai is pack size mein.', style: TextStyle(fontSize: 12, color: Colors.grey.shade400)))
                    else ...items.map((item) => Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12),
                        border: Border.all(color: Colors.grey.shade200),
                        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.04), blurRadius: 6)]),
                      child: Row(children: [
                        Icon(item.hasBarcode ? Icons.qr_code_rounded : Icons.inventory_2_outlined, color: AppColors.blue, size: 20),
                        const SizedBox(width: 10),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(item.displayName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                          Text(item.hasBarcode ? 'Barcode: ${item.crn}' : 'No Barcode (bulk entry)',
                            style: TextStyle(fontSize: 11, color: item.hasBarcode ? AppColors.textGrey : Colors.grey.shade400, fontFamily: item.hasBarcode ? 'monospace' : null)),
                        ])),
                        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                          Text('₹${item.price.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: AppColors.red)),
                          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                            decoration: BoxDecoration(
                              color: item.quantity <= 0 ? AppColors.red : AppColors.success,
...
class _TotalSellScreenState extends State<TotalSellScreen> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final sessions = await SalesService.instance.getSessionsByDate(_date);
    if (!mounted) return;
    setState(() {
      _sessions = sessions;
      _total = sessions.fold<double>(0, (s, i) => s + i.totalAmount);
      _loading = false;
    });
  }
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    body: Column(children: [
      Container(margin: const EdgeInsets.all(16), padding: const EdgeInsets.all(16),
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_isToday(_date) ? "Today's Total" : _fmtDate(_date), style: const TextStyle(color: Colors.white70, fontSize: 12)),
            Text('₹${_total.toStringAsFixed(0)}', style: const TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.w900)),
            Text('${_sessions.length} transactions', style: const TextStyle(color: Colors.white70, fontSize: 12)),
          ])),
          GestureDetector(
            onTap: () async {
      ...
      Expanded(child: _loading ? buildLoader() : _sessions.isEmpty
...
class _WorkerHomeScreenState extends State<WorkerHomeScreen> {
  ...
  Future<void> _load() async {
    ...
    if (s == null) return;
    setState(() => _loading = true);
    final sessions = await SalesService.instance.getTodaySessions(workerId: s.userId);
    ...
    if (!mounted) return;
    setState(() { _sessions = sessions; _returns = rets.length; _pending = pending; _loading = false; });
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    final itemsSold = _sessions.fold<int>(0, (s, i) => s + i.itemCount);
    final earned = _sessions.fold<double>(0, (s, i) => s + i.totalAmount);
    return Scaffold(
      body: RefreshIndicator(onRefresh: _load, color: AppColors.red,
        child: CustomScrollView(slivers: [
          ...
          SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
            ...
            if (!_loading) Row(children: [
              Expanded(child: _mini('Sold Today', '$itemsSold', Icons.shopping_bag_rounded, AppColors.success)),
              const SizedBox(width: 10),
            ...
            else if (_sessions.isEmpty) buildEmpty('Abhi tak koi sell nahi.', Icons.shopping_bag_rounded)
            else ..._sessions.take(5).map((s) => _SessionCard(session: s)),
          ]))),
        ])));
  }
...
// Ek poore checkout (Sale Session) ka card — tap karke andar ke
// individual items expand ho jaate hain
...
class _SessionCardState extends State<_SessionCard> {
  ...
  List<SaleRecord>? _lines;
  bool _loadingLines = false;

  Future<void> _toggle() async {
    if (!_expanded && _lines == null) {
      setState(() => _loadingLines = true);
      final lines = await SalesService.instance.getLinesForSession(widget.session.id!);
      if (!mounted) return;
      setState(() { _lines = lines; _loadingLines = false; });
    }
      ...
      child: Column(children: [
        InkWell(
          ...
          child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
            Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.success.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.receipt_long_rounded, color: AppColors.success)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${s.itemCount} item${s.itemCount > 1 ? "s" : ""} sold', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
              Text('${s.workerName} • ${_fmtDate(s.soldAt)} ${_fmtTime(s.soldAt)}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
              const SizedBox(height: 4),
              Wrap(spacing: 4, children: [
                if (s.isCredit) chip('UDHARI • ${s.creditCustomer ?? ""}', AppColors.warning)
                else chip(s.paymentMethod == 'online' ? 'ONLINE' : 'COD', s.paymentMethod == 'online' ? AppColors.success : AppColors.blue),
                if (!s.isSynced) chip('unsynced', AppColors.textGrey),
              ]),
            ])),
            const SizedBox(width: 8),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('₹${s.totalAmount.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: AppColors.success)),
              Icon(_expanded ? Icons.expand_less_rounded : Icons.expand_more_rounded, color: AppColors.textGrey),
            ]),
          ]))),
        if (_expanded) Padding(padding: const EdgeInsets.fromLTRB(14, 0, 14, 14),
          child: _loadingLines
            ...
            : Column(children: (_lines ?? []).map((l) => Container(
                margin: const EdgeInsets.only(top: 6), padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: AppColors.bgLight, borderRadius: BorderRadius.circular(10)),
                child: Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(l.itemName, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 12)),
                    Text('${l.crn.isEmpty ? "No Barcode" : l.crn} • Pack Size: ${l.itemPackSize}${l.discount > 0 ? " • Disc: ₹${l.discount.toStringAsFixed(0)}" : ""}',
                      style: const TextStyle(fontSize: 10, color: AppColors.textGrey)),
                  ])),
                  Text('₹${l.totalAmount.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                ]))).toList())),
      ]));
  }
}
              ...
              Container(constraints: const BoxConstraints(maxHeight: 180),
                ...
                child: ListView.builder(shrinkWrap: true, itemCount: _suggestions.length, itemBuilder: (_, i) {
                  ...
                  return ListTile(dense: true,
                    leading: const Icon(Icons.shopping_basket_rounded, color: AppColors.red, size: 18),
                    title: Text(it.displayName, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                    subtitle: Text('${it.displayCrn} • ₹${it.price.toStringAsFixed(0)} • Qty:${it.quantity}', style: const TextStyle(color: Colors.white54, fontSize: 11)),
                    onTap: () => _select(it));
                })),
...
// ══════════════════════
//  WORKER MANUAL SEARCH — Unit search karo, tap karke kholo,
//  andar multi-select karke seedha Sell Window pe bhejo
// ══════════════════════
...
class _WorkerManualSearchScreenState extends State<WorkerManualSearchScreen> {
  final _ctrl = TextEditingController();
  List<InventoryUnit> _allUnits = [];
  List<InventoryUnit> _results = [];
  Map<int, List<InventorySubUnit>> _subUnitsMap = {};
  bool _loading = true;
  bool _loaded = false;
  bool _addMoreMode = false;
  ...
  void didChangeDependencies() {
    ...
    if (!_loaded) {
      _loaded = true;
      _addMoreMode = ModalRoute.of(context)?.settings.arguments == 'add_more';
      _load();
    }
  }
  ...
  Future<void> _load() async {
    ...
    final subMap = await InventoryService.instance.getSubUnitsGroupedByUnit();
    if (!mounted) return;
    setState(() { _allUnits = units; _results = units; _subUnitsMap = subMap; _loading = false; });
  }
  ...
  void _onType(String q) {
    setState(() {
      _results = q.trim().isEmpty
        ? _allUnits
        : _allUnits.where((u) => u.name.toLowerCase().contains(q.trim().toLowerCase())).toList();
    });
  }

  @override
  Widget build(BuildContext ctx) => Scaffold(
    appBar: const GradientAppBar(title: 'Manual Search'),
    body: Column(children: [
      Container(color: Colors.white, padding: const EdgeInsets.all(16),
        child: TextField(controller: _ctrl, onChanged: _onType,
          decoration: const InputDecoration(
            hintText: 'Item search karo (jaise: Namak, Biscuit)...',
            prefixIcon: Icon(Icons.search_rounded, color: AppColors.blue)))),
      Expanded(child: _loading ? buildLoader()
        : _results.isEmpty ? buildEmpty('Koi unit nahi mila.', Icons.search_off_rounded)
        : ListView.builder(
            padding: const EdgeInsets.all(16),
            itemCount: _results.length,
            itemBuilder: (_, i) {
              final unit = _results[i];
              final subs = _subUnitsMap[unit.id] ?? [];
              return GestureDetector(
                onTap: () => Navigator.push(ctx, MaterialPageRoute(
                  builder: (_) => WorkerUnitPickScreen(unit: unit, addMoreMode: _addMoreMode))),
                child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                  ...
                  child: Row(children: [
                    Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.blue.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
                      child: const Icon(Icons.category_rounded, color: AppColors.blue, size: 24)),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(unit.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                      Text('${unit.category} • ${subs.isEmpty ? "Koi pack size nahi" : "Pack Sizes: ${subs.map((s) => s.packSize).join(", ")}"}',
                        style: const TextStyle(fontSize: 12, color: AppColors.textGrey)),
                    ])),
                    const Icon(Icons.chevron_right_rounded, color: AppColors.textGrey),
                  ])));
...
// Unit ke andar items — tap karke multi-select karo, phir seedha Sell Window
class WorkerUnitPickScreen extends StatefulWidget {
  final InventoryUnit unit;
  final bool addMoreMode;
  const WorkerUnitPickScreen({super.key, required this.unit, this.addMoreMode = false});
  @override State<WorkerUnitPickScreen> createState() => _WorkerUnitPickScreenState();
}
class _WorkerUnitPickScreenState extends State<WorkerUnitPickScreen> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final items = await InventoryService.instance.getAllItems(unit: widget.unit.name, category: widget.unit.category);
    if (!mounted) return;
  ...
  void _confirm() {
    ...
    if (widget.addMoreMode) {
      // Cart mein wapas jaana hai — poori search chain se pop karke
      // seedha WorkerSellScreen ko result de do
      Navigator.pop(context);
      Navigator.pop(context, selected);
    } else {
      // Fresh flow — search screens hata kar seedha Sell Window pe jao
      Navigator.of(context).pushNamedAndRemoveUntil(
        '/worker-sell', (route) => route.settings.name == '/worker-home',
        arguments: selected);
    }
  }
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    floatingActionButton: _selectedIds.isEmpty ? null : FloatingActionButton.extended(
      onPressed: _confirm, backgroundColor: AppColors.success,
      icon: const Icon(Icons.point_of_sale_rounded),
      label: Text('Sell Window (${_selectedIds.length})')),
    body: _loading ? buildLoader() : _items.isEmpty
      ? buildEmpty('Is unit mein koi item nahi hai.', Icons.inventory_2_outlined)
      : ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(
              ...
              child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                  color: outOfStock ? Colors.grey.shade50 : (selected ? AppColors.success.withValues(alpha: 0.08) : Colors.white),
                  borderRadius: BorderRadius.circular(16),
                  border: selected ? Border.all(color: AppColors.success, width: 2) : null,
                  boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.06), blurRadius: 10)]),
                child: Row(children: [
                  Icon(selected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded,
                    color: outOfStock ? Colors.grey.shade300 : (selected ? AppColors.success : AppColors.textGrey)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(item.displayName, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: outOfStock ? AppColors.textGrey : AppColors.textDark)),
                    Text('${item.displayCrn} • ${item.brand} • ${item.packSize}', style: const TextStyle(fontSize: 12, color: AppColors.textGrey)),
                    Text(outOfStock ? 'Out of Stock' : 'Available: ${item.quantity}',
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: outOfStock ? AppColors.red : AppColors.success)),
                  ])),
...
class _WorkerSellScreenState extends State<WorkerSellScreen> {
  ...
  // Payment method — default COD, worker "Online" tick karta hai to online ho jata hai
  ...
  void didChangeDependencies() {
    ...
    if (!_loaded) {
      ...
      if (args is List) {
        for (final a in args) {
          if (a is InventoryItem && !_cart.any((c) => c.item.id == a.id)) {
  ...
  @override
  void dispose() { _creditNameCtrl.dispose(); super.dispose(); }

  double get _grandTotal => _cart.fold(0.0, (s, c) => s + c.total);

  void _addItemsToCart(List<InventoryItem> items) {
    int addedCount = 0;
    ...
    if (addedCount > 0) setState(() {});
  }

  Future<void> _addMore() async {
    // Worker ko choose karne do — Scan karke add karein ya Search karke
    final choice = await showModalBottomSheet<String>(
      ...
      builder: (bCtx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 16),
        ListTile(leading: const Icon(Icons.qr_code_scanner_rounded, color: AppColors.blue),
          title: const Text('Scan Karke Add Karo'), onTap: () => Navigator.pop(bCtx, 'scan')),
        ListTile(leading: const Icon(Icons.search_rounded, color: AppColors.red),
          title: const Text('Search Karke Add Karo'), onTap: () => Navigator.pop(bCtx, 'search')),
        const SizedBox(height: 10),
      ])));

    if (choice == 'scan') {
      final result = await Navigator.pushNamed(context, '/worker-scanner', arguments: 'add_more');
      if (result is InventoryItem) {
        if (_cart.any((c) => c.item.id == result.id)) {
      ...
      final result = await Navigator.pushNamed(context, '/worker-manual-search', arguments: 'add_more');
      if (result is List<InventoryItem>) _addItemsToCart(result);
    }
  }
  ...
  Future<void> _completeSell() async {
    ...
    if (_isCredit && _creditNameCtrl.text.trim().isEmpty) {
      showSnack(context, 'Udhari ke liye customer ka naam daalo!', isError: true);
      return;
    }
    setState(() => _selling = true);
    final r = await SalesService.instance.completeSale(
      lines: _cart.map((ci) => SaleLineInput(itemId: ci.item.id!, discount: ci.discount)).toList(),
      worker: session,
      paymentMethod: _paymentMethod,
      isCredit: _isCredit,
      creditCustomer: _isCredit ? _creditNameCtrl.text.trim() : null,
    );
    if (!mounted) return;
    setState(() => _selling = false);
    if (r.isSuccess) {
      final totalCollected = r.data!.totalAmount;
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 6),
            if (_isCredit)
              chip('UDHARI • ${_creditNameCtrl.text.trim()}', AppColors.warning)
            else
              chip(_paymentMethod == 'online' ? 'ONLINE PAYMENT' : 'CASH ON DELIVERY (COD)', _paymentMethod == 'online' ? AppColors.success : AppColors.blue),
          ]),
                  ...
                  Row(children: [
                    Container(padding: const EdgeInsets.all(10),
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(10)),
                      child: const Icon(Icons.shopping_basket_rounded, color: AppColors.blue, size: 22)),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(ci.item.displayName,
                        style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                      Text('${ci.item.displayCrn} \u2022 ${ci.item.brand} \u2022 ${ci.item.packSize}',
            ...
            child: Column(children: [
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _isCredit ? AppColors.warning.withValues(alpha: 0.1) : Colors.transparent,
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: _isCredit ? AppColors.warning : Colors.grey.shade300)),
                  child: Row(children: [
                    Checkbox(value: _isCredit, activeColor: AppColors.warning,
                      onChanged: (v) => setState(() => _isCredit = v ?? false)),
                    const Text('Udhari (Credit Sale)', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                  ]))),
              if (_isCredit) ...[
                const SizedBox(height: 8),
                TextField(controller: _creditNameCtrl,
                  decoration: const InputDecoration(labelText: 'Customer ka Naam', prefixIcon: Icon(Icons.person_rounded, color: AppColors.warning))),
              ],
              const SizedBox(height: 10),
              // Payment method — Online / COD
              Row(children: [
                const Text('Payment:', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textGrey)),
                const SizedBox(width: 10),
                Expanded(child: GestureDetector(
                  onTap: () => setState(() => _paymentMethod = 'cod'),
                  child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
                    decoration: BoxDecoration(
                      color: _paymentMethod == 'cod' ? AppColors.blue : Colors.grey.shade100,
                      borderRadius: BorderRadius.circular(10)),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.money_rounded, size: 16, color: _paymentMethod == 'cod' ? Colors.white : AppColors.textGrey),
                      const SizedBox(width: 6),
                      Text('COD', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: _paymentMethod == 'cod' ? Colors.white : AppColors.textGrey)),
                    ])))),
                const SizedBox(width: 8),
                Expanded(child: GestureDetector(
                  ...
                  child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
                    decoration: BoxDecoration(
                      color: _paymentMethod == 'online' ? AppColors.success : Colors.grey.shade100,
                      borderRadius: BorderRadius.circular(10)),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.qr_code_rounded, size: 16, color: _paymentMethod == 'online' ? Colors.white : AppColors.textGrey),
                      const SizedBox(width: 6),
                      Text('Online', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: _paymentMethod == 'online' ? Colors.white : AppColors.textGrey)),
                    ])))),
              ]),
              const SizedBox(height: 10),
              // Grand total
...
class _WorkerHistoryScreenState extends State<WorkerHistoryScreen> {
  ...
  @override
  Widget build(BuildContext ctx) {
    final total = _sessions.fold<double>(0, (s, i) => s + i.totalAmount);
    final itemsSold = _sessions.fold<int>(0, (s, i) => s + i.itemCount);
    return Scaffold(
        ...
        Container(margin: const EdgeInsets.all(16), padding: const EdgeInsets.all(16),
          ...
          child: Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              Text('$itemsSold', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 26)),
            ]),
          ])),
        Expanded(child: _loading ? buildLoader() : _sessions.isEmpty
...
class _InventoryAlertScreenState extends State<InventoryAlertScreen> {
  // unit-label → sub-units map
  Map<String, List<InventorySubUnit>> _outMap = {};
  Map<String, List<InventorySubUnit>> _lowMap = {};
  bool _loading = true;
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final rows = await InventoryService.instance.getLowStockSubUnits();

    final Map<String, List<InventorySubUnit>> outMap = {};
    final Map<String, List<InventorySubUnit>> lowMap = {};

    for (final r in rows) {
      final su = InventorySubUnit.fromMap(r);
      final unitLabel = '${r['unit_name']} (${r['unit_category']})';
      if (su.isOutOfStock) {
        outMap.putIfAbsent(unitLabel, () => []).add(su);
      } else {
        lowMap.putIfAbsent(unitLabel, () => []).add(su);
      }
    }
        ...
        : RefreshIndicator(onRefresh: _load, color: AppColors.red,
            child: ListView(padding: const EdgeInsets.all(16), children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: _statBox('Low Stock', _lowMap.values.fold(0, (s,l) => s+l.length), AppColors.warning, Icons.warning_rounded)),
              ]),
              const SizedBox(height: 20),
              Text('Har pack size (block) ka apna threshold hota hai — "Unit Banao" screen mein pack size pe tap karke set kar sakte ho.',
              ...
              if (_outMap.isNotEmpty) ...[
                ...
                const SizedBox(height: 10),
                ..._outMap.entries.map((e) => _UnitAlertGroup(unitName: e.key, subUnits: e.value, color: AppColors.red)),
                const SizedBox(height: 16),
              ],

              if (_lowMap.isNotEmpty) ...[
                _sectionHdr('🟡 Low Stock', AppColors.warning),
                const SizedBox(height: 10),
                ..._lowMap.entries.map((e) => _UnitAlertGroup(unitName: e.key, subUnits: e.value, color: AppColors.warning)),
              ],
            ])));
...
class _UnitAlertGroup extends StatelessWidget {
  ...
  final List<InventorySubUnit> subUnits;
  final Color color;
  const _UnitAlertGroup({required this.unitName, required this.subUnits, required this.color});
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        ...
        child: Row(children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            ...
            child: Text('${subUnits.length} pack size(s)', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700))),
        ])),
      // Pack Sizes list — ab block (pack size) level pe dikh raha hai, item level pe nahi
      ...subUnits.map((su) => Container(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade100))),
        child: Row(children: [
          const SizedBox(width: 4),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Pack Size: ${su.packSize}', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
            Text('Alert threshold: ${su.lowStockThreshold}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(20)),
            child: Text(su.isOutOfStock ? 'OUT' : '${su.totalQuantity} pcs',
...
// ══════════════════════
//  UDHARI (CREDIT LEDGER) SCREENS
// ══════════════════════
String _fmtDate(DateTime d) => '${d.day}/${d.month}/${d.year}';
String _fmtTime(DateTime d) => '${d.hour}:${d.minute.toString().padLeft(2, '0')}';

class UdhariScreen extends StatefulWidget {
  const UdhariScreen({super.key});
  @override State<UdhariScreen> createState() => _UdhariScreenState();
}

class _UdhariScreenState extends State<UdhariScreen> {
  List<CreditSummary> _summaries = [];
  bool _loading = true;
  String _query = '';
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final s = await CreditService.instance.getAllSummaries();
    if (!mounted) return;
    setState(() { _summaries = s; _loading = false; });
  }

  List<CreditSummary> get _filtered => _query.trim().isEmpty
      ? _summaries
      : _summaries.where((s) => s.account.customerName.toLowerCase().contains(_query.toLowerCase())).toList();

  double get _totalOutstanding => _summaries.fold(0.0, (sum, s) => sum + s.balance);

  void _showNewUdhariDialog() {
    final nameCtrl = TextEditingController();
    final amtCtrl  = TextEditingController();
    showDialog(context: context, builder: (dCtx) => AlertDialog(
      title: const Text('Nayi Udhari Entry'),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Customer Naam')),
        const SizedBox(height: 10),
        TextField(controller: amtCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Udhari Amount (₹)')),
      ]),
      actions: [
        TextButton(onPressed: () => Navigator.pop(dCtx), child: const Text('Cancel')),
        ElevatedButton(onPressed: () async {
          ...
          if (name.isEmpty || amt <= 0) { showSnack(dCtx, 'Sahi naam aur amount daalo', isError: true); return; }
          final r = await CreditService.instance.addDebit(
            customerName: name, amount: amt, note: 'Manual entry', worker: AuthService.instance.currentSession);
          Navigator.pop(dCtx);
          if (!mounted) return;
          if (r.isSuccess) { showSnack(context, 'Udhari add ho gayi ✓'); _load(); }
          else { showSnack(context, r.error!, isError: true); }
        }, child: const Text('Save')),
      ]));
  }
  ...
  @override
  Widget build(BuildContext ctx) => Scaffold(
    appBar: const GradientAppBar(title: 'Udhari'),
    floatingActionButton: FloatingActionButton.extended(
      onPressed: _showNewUdhariDialog, backgroundColor: AppColors.red,
      icon: const Icon(Icons.add), label: const Text('Nayi Udhari')),
    body: _loading ? buildLoader() : RefreshIndicator(onRefresh: _load, color: AppColors.red,
      child: ListView(padding: const EdgeInsets.all(16), children: [
        Container(padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(gradient: AppColors.gradientRB, borderRadius: BorderRadius.circular(16),
            boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.15), blurRadius: 12, offset: const Offset(0, 4))]),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Total Outstanding Udhari', style: TextStyle(color: Colors.white70, fontSize: 13)),
            const SizedBox(height: 6),
            Text('₹${_totalOutstanding.toStringAsFixed(0)}', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text('${_summaries.where((s) => s.balance > 0).length} customers ka udhar baki hai', style: const TextStyle(color: Colors.white70, fontSize: 12)),
          ])),
        const SizedBox(height: 16),
        TextField(
          decoration: const InputDecoration(labelText: 'Customer search karo', prefixIcon: Icon(Icons.search_rounded, color: AppColors.red)),
          onChanged: (v) => setState(() => _query = v)),
        const SizedBox(height: 14),
        if (_filtered.isEmpty) buildEmpty('Koi udhari record nahi.\n"Nayi Udhari" se add karo.', Icons.account_balance_wallet_outlined),
        ..._filtered.map((s) => GestureDetector(
          onTap: () => Navigator.push(ctx, MaterialPageRoute(builder: (_) => UdhariCustomerScreen(summary: s))).then((_) => _load()),
          child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14),
              boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 8)]),
            child: Row(children: [
              Container(width: 44, height: 44,
                decoration: BoxDecoration(color: (s.balance > 0 ? AppColors.warning : AppColors.success).withValues(alpha: 0.12), shape: BoxShape.circle),
                child: Icon(Icons.person_rounded, color: s.balance > 0 ? AppColors.warning : AppColors.success)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(s.account.customerName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                if (s.lastTxnDate != null)
                  Text('Last: ${_fmtDate(s.lastTxnDate!)}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
              ])),
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('₹${s.balance.toStringAsFixed(0)}',
                  style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: s.balance > 0 ? AppColors.warning : AppColors.success)),
                Text(s.balance > 0 ? 'Baaki' : 'Clear', style: const TextStyle(fontSize: 10, color: AppColors.textGrey)),
              ]),
            ]))))
      ])));
}

class UdhariCustomerScreen extends StatefulWidget {
  final CreditSummary summary;
  const UdhariCustomerScreen({super.key, required this.summary});
  @override State<UdhariCustomerScreen> createState() => _UdhariCustomerScreenState();
}

class _UdhariCustomerScreenState extends State<UdhariCustomerScreen> {
  List<CreditTransaction> _txns = [];
  bool _loading = true;
  late CreditSummary _summary;

  @override void initState() { super.initState(); _summary = widget.summary; _load(); }

  Future<void> _load() async {
    setState(() => _loading = true);
    final t = await CreditService.instance.getTransactions(_summary.account.id!);
    final all = await CreditService.instance.getAllSummaries();
    final updated = all.where((s) => s.account.id == _summary.account.id).toList();
    if (!mounted) return;
    setState(() {
      _txns = t;
      if (updated.isNotEmpty) _summary = updated.first;
      _loading = false;
    });
  }
  ...
  void _showReceiveDialog() {
    ...
    showDialog(context: context, builder: (dCtx) => AlertDialog(
      title: Text('Receive Payment — ${_summary.account.customerName}'),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        Text('Total Udhari: ₹${_summary.balance.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.textGrey, fontSize: 12)),
        const SizedBox(height: 10),
        TextField(controller: amtCtrl, keyboardType: TextInputType.number, autofocus: true,
          decoration: const InputDecoration(labelText: 'Receive Amount (₹)', prefixIcon: Icon(Icons.currency_rupee_rounded, color: AppColors.success))),
        const SizedBox(height: 10),
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          onPressed: () async {
            final amt = double.tryParse(amtCtrl.text.trim()) ?? 0;
            if (amt <= 0) { showSnack(dCtx, 'Sahi amount daalo', isError: true); return; }
            final r = await CreditService.instance.receivePayment(
              accountId: _summary.account.id!, amount: amt, note: noteCtrl.text.trim(),
              worker: AuthService.instance.currentSession);
            Navigator.pop(dCtx);
            if (!mounted) return;
            if (r.isSuccess) { showSnack(context, 'Payment record ho gaya ✓'); _load(); }
            else { showSnack(context, r.error!, isError: true); }
  ...
  @override
  Widget build(BuildContext ctx) => Scaffold(
    appBar: GradientAppBar(title: _summary.account.customerName),
    floatingActionButton: _summary.balance > 0 ? FloatingActionButton.extended(
      onPressed: _showReceiveDialog, backgroundColor: AppColors.success,
      icon: const Icon(Icons.download_done_rounded), label: const Text('Receive Udhari')) : null,
    body: _loading ? buildLoader() : RefreshIndicator(onRefresh: _load, color: AppColors.red,
      child: ListView(padding: const EdgeInsets.all(16), children: [
        Container(padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: _summary.balance > 0 ? [const Color(0xFFF57C00), const Color(0xFFFFB74D)] : [AppColors.success, const Color(0xFF81C784)],
              begin: Alignment.topLeft, end: Alignment.bottomRight),
            borderRadius: BorderRadius.circular(16)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Total Udhari (Baaki Amount)', style: TextStyle(color: Colors.white70, fontSize: 13)),
            const SizedBox(height: 6),
            Text('₹${_summary.balance.toStringAsFixed(0)}', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w800)),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _miniStat('Udhar Diya', '₹${_summary.totalDebit.toStringAsFixed(0)}')),
              Expanded(child: _miniStat('Wapas Mila', '₹${_summary.totalCredit.toStringAsFixed(0)}')),
            ]),
          ])),
        const SizedBox(height: 18),
        const SectionTitle(text: 'Date-wise Detail'),
        const SizedBox(height: 12),
        if (_txns.isEmpty) buildEmpty('Koi transaction nahi.', Icons.receipt_long_outlined),
        ..._txns.map((t) => Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14),
            boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 8)]),
          child: Row(children: [
            Container(padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: (t.isDebit ? AppColors.warning : AppColors.success).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10)),
              child: Icon(t.isDebit ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded, size: 18, color: t.isDebit ? AppColors.warning : AppColors.success)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(t.isDebit ? 'Udhar Diya' : 'Payment Mila (Receive Udhari)', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
              if (t.note.isNotEmpty) Text(t.note, style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
              Text('${_fmtDate(t.txnDate)} • ${_fmtTime(t.txnDate)}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
            ])),
            Text('${t.isDebit ? "+" : "-"}₹${t.amount.toStringAsFixed(0)}',
              style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: t.isDebit ? AppColors.warning : AppColors.success)),
          ]))),
      ])));
