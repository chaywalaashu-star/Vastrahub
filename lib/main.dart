...
class _InventorySetupScreenState extends State<InventorySetupScreen> with SingleTickerProviderStateMixin {
  ...
  final _uQty   = TextEditingController(text: '0');
  String _uGender = 'Men';
  ...
  @override void dispose() { _tab.dispose(); _uName.dispose(); _uSizes.dispose(); _uQty.dispose(); _iName.dispose(); _iCrn.dispose(); _iPrice.dispose(); _iQty.dispose(); super.dispose(); }
  ...
  Future<void> _loadUnits() async {
    ...
    setState(() {
      _units = u;
      _subUnitsMap = subMap;
      if (u.isNotEmpty && _selUnit.isEmpty) _selUnit = u.first.name;
      _syncSelectedSubUnit();
    });
  }

  void _syncSelectedSubUnit() {
    final unit = _units.where((u) => u.name == _selUnit).toList();
  ...
  // Unit banao — agar naam pehle se hai to REPLACE nahi hota, balki
  // diya gaya size us existing unit ke andar nayi SUB-UNIT ban kar jud jaata hai.
  Future<void> _saveUnit() async {
    ...
    final r = await InventoryService.instance.addOrUpdateUnit(
      ...
      quantityPerSize: int.tryParse(_uQty.text) ?? 0);
    if (!mounted) return;
    setState(() => _savingUnit = false);
    if (r.isSuccess) {
      showSnack(context, 'Saved! "${_uName.text.trim()}" mein size "${_uSizes.text.trim()}" add ho gayi ✓');
  ...
  Widget _unitTab() => ListView(padding: const EdgeInsets.all(16), children: [
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 12),
      TextField(controller: _uQty, keyboardType: TextInputType.number,
        decoration: const InputDecoration(labelText: 'Stock Qty (is size ki, optional)', prefixIcon: Icon(Icons.numbers_rounded, color: AppColors.red))),
      const SizedBox(height: 10),
      Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.blue.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(10)),
        child: const Text('Same unit naam dobara likhoge to purani unit REPLACE nahi hogi — nayi size uske andar SUB-UNIT ban kar jud jayegi.', style: TextStyle(fontSize: 12, color: AppColors.blue))),
      const SizedBox(height: 16),
      ...
      return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          subs.isEmpty
            ...
            : Wrap(spacing: 6, runSpacing: 6, children: subs.map((s) => Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                decoration: BoxDecoration(color: AppColors.bgLight, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade300)),
                child: Text('${s.size}${s.totalQuantity > 0 ? " • ${s.totalQuantity}" : ""}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textDark)))).toList()),
        ]));
    }),
  ]);
    ...
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_units.isEmpty)
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (v) => setState(() { _selUnit = v!; _syncSelectedSubUnit(); })),
      const SizedBox(height: 12),
      Builder(builder: (_) {
        final unit = _units.where((u) => u.name == _selUnit).toList();
        final subs = unit.isEmpty ? <InventorySubUnit>[] : (_subUnitsMap[unit.first.id] ?? []);
        ...
        return DropdownButtonFormField<String>(
          ...
          items: subs.map((s) => DropdownMenuItem(value: s.size, child: Text(s.size))).toList(),
...
class _WorkerUnitPickScreenState extends State<WorkerUnitPickScreen> {
  List<InventoryItem> _items = [];
  final Set<String> _selectedCrns = {};
  bool _loading = true;
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final items = await InventoryService.instance.getAllItems(unit: widget.unit.name);
    if (!mounted) return;
    setState(() { _items = items; _loading = false; });
  }
  ...
  void _toggle(InventoryItem item) {
    ...
    setState(() {
      if (_selectedCrns.contains(item.crn)) { _selectedCrns.remove(item.crn); }
      else { _selectedCrns.add(item.crn); }
    });
  }

  void _confirm() {
    final selected = _items.where((i) => _selectedCrns.contains(i.crn)).toList();
  ...
  Widget build(BuildContext ctx) => Scaffold(
    appBar: GradientAppBar(title: widget.unit.name),
    floatingActionButton: _selectedCrns.isEmpty ? null : FloatingActionButton.extended(
      onPressed: _confirm, backgroundColor: AppColors.success,
      icon: const Icon(Icons.point_of_sale_rounded),
      label: Text('Sell Window (${_selectedCrns.length})')),
    body: _loading ? buildLoader() : _items.isEmpty
      ...
      : ListView.builder(
          ...
          itemBuilder: (_, i) {
            final item = _items[i];
            final selected = _selectedCrns.contains(item.crn);
            ...
            return GestureDetector(
              ...
              child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(item.name, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: outOfStock ? AppColors.textGrey : AppColors.textDark)),
...
class _WorkerSellScreenState extends State<WorkerSellScreen> {
  ...
  void _addItemsToCart(List<InventoryItem> items) {
    ...
    for (final item in items) {
      if (!_cart.any((c) => c.item.crn == item.crn)) {
