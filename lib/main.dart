...
class _WorkerScannerScreenState extends State<WorkerScannerScreen> {
  ...
  @override void dispose() { _ctrl.dispose(); _scanCtrl.dispose(); _debounce?.cancel(); super.dispose(); }
  ...
  void _select(InventoryItem item) {
    _ctrl.clear(); setState(() => _suggestions = []);
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    body: Stack(children: [
      ...
      Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 16),
        const Text('QR ya barcode scan karo', style: TextStyle(color: Colors.white70, fontSize: 13)),
      ])),

      // Bottom: live search + manual entry
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Padding(padding: const EdgeInsets.all(16), child: Column(children: [
              const Text('Ya CRN / naam type karo:', style: TextStyle(color: Colors.white60, fontSize: 12)),
              const SizedBox(height: 8),
              TextField(controller: _ctrl, style: const TextStyle(color: Colors.white),
                ...
                decoration: InputDecoration(
                  hintText: 'Naam ya CRN likhte jao — results aayenge...',
...
class _WorkerSellScreenState extends State<WorkerSellScreen> {
  ...
  Future<void> _addMore() async {
    final result = await Navigator.pushNamed(context, '/worker-manual-search');
...
class _InventoryAlertScreenState extends State<InventoryAlertScreen> {
  List<InventoryItem> _lowItems = [];
  List<InventoryItem> _outItems = [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _outItems  = all.where((i) => i.quantity == 0).toList();
      _lowItems  = all.where((i) => i.quantity > 0 && i.quantity <= 5).toList();
      _loading   = false;
    });
  }
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    body: _loading ? buildLoader() : RefreshIndicator(
      onRefresh: _load, color: AppColors.red,
      child: (_lowItems.isEmpty && _outItems.isEmpty)
        ? buildEmpty('Sab theek hai! ✅\nKoi item low stock mein nahi.', Icons.check_circle_rounded)
        : ListView(padding: const EdgeInsets.all(16), children: [

          // Summary cards
          Row(children: [
            Expanded(child: _alertStat('Out of Stock', '${_outItems.length}', Icons.remove_shopping_cart_rounded, AppColors.red)),
            const SizedBox(width: 12),
            Expanded(child: _alertStat('Low Stock (≤5)', '${_lowItems.length}', Icons.warning_rounded, AppColors.warning)),
          ]),
          const SizedBox(height: 20),
          ...
          if (_outItems.isNotEmpty) ...[
            Row(children: [
              ...
              const Text('Out of Stock', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.red)),
            ]),
            const SizedBox(height: 10),
            ..._outItems.map((i) => _AlertCard(item: i, isOut: true)),
            const SizedBox(height: 16),
          ],
          ...
          if (_lowItems.isNotEmpty) ...[
            Row(children: [
              ...
              const SizedBox(width: 8),
              const Text('Low Stock (5 ya kam)', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.warning)),
            ]),
            const SizedBox(height: 10),
            ..._lowItems.map((i) => _AlertCard(item: i, isOut: false)),
          ],
        ])));

  Widget _alertStat(String l, String v, IconData icon, Color c) => Container(
    padding: const EdgeInsets.all(16),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Icon(icon, color: c, size: 24), const SizedBox(height: 8),
      Text(v, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: c)),
      Text(l, style: TextStyle(fontSize: 12, color: c.withValues(alpha: 0.8), fontWeight: FontWeight.w600)),
    ]));
}
...
class _AlertCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext ctx) {
    final color = isOut ? AppColors.red : AppColors.warning;
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      ...
      child: Row(children: [
        Container(width: 46, height: 46,
          ...
          child: Icon(isOut ? Icons.remove_shopping_cart_rounded : Icons.warning_amber_rounded, color: color, size: 22)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(item.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
          const SizedBox(height: 3),
          Text('CRN: ${item.crn} • ${item.brand} • ${item.size}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
          Text('${item.gender} • ${item.unit}', style: const TextStyle(fontSize: 11, color: AppColors.textGrey)),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
            decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(20)),
            child: Text(isOut ? 'OUT' : 'Qty: ${item.quantity}',
              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12))),
          const SizedBox(height: 4),
          Text('₹${item.price.toStringAsFixed(0)}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textGrey)),
        ]),
      ]));
  }
}
