...
class _InventorySetupScreenState extends State<InventorySetupScreen> with SingleTickerProviderStateMixin {
  ...
  final _uSizes = TextEditingController();
  String _uGender = 'Men';
  ...
  // Unit + Size + Quantity hi asal mein zaroori hain — quantity yahi se
  ...
  void _syncSelectedSubUnit() {
    ...
    if (subs.isEmpty) {
      _selSubUnit = '';
    } else if (!subs.any((s) => s.size == _selSubUnit)) {
      _selSubUnit = subs.first.size;
    }
  }

  // Unit banao — sirf DHAANCHA (structure) banta hai, koi stock quantity
  // yahan nahi daalte. Agar naam+gender pehle se hai to REPLACE nahi hota,
  // balki diya gaya size us existing unit ke andar nayi SUB-UNIT ban kar
  ...
  Future<void> _saveUnit() async {
    ...
    if (_uSizes.text.trim().isEmpty) { showSnack(context, 'Size daalo! (jaise: M ya 32)', isError: true); return; }
    setState(() => _savingUnit = true);
    final r = await InventoryService.instance.addOrUpdateUnit(
      name: _uName.text.trim(), gender: _uGender,
      sizesInput: _uSizes.text.trim());
    if (!mounted) return;
    setState(() => _savingUnit = false);
    if (r.isSuccess) {
      showSnack(context, 'Saved! "${_uName.text.trim()}" ($_uGender) mein size "${_uSizes.text.trim()}" ban gayi ✓');
      // Unit naam jaan bujh kar clear nahi karte — taaki isi unit mein
      // agli size turant add kar sako. Sirf size field clear hoga.
      _uSizes.clear();
      _loadUnits();
    } else {
  ...
  Future<void> _editThreshold(InventorySubUnit su) async {
    ...
    final result = await showDialog<int>(context: context, builder: (dCtx) => AlertDialog(
      title: Text('Low-Stock Alert — Size "${su.size}"'),
  ...
  Future<void> _saveItem() async {
    ...
    if (_selSubUnit.isEmpty) { showSnack(context, 'Pehle is unit mein ek size (sub-unit) banao!', isError: true); return; }
    ...
    final r = await InventoryService.instance.addItem(InventoryItem(
      ...
      gender: unit.gender, size: _selSubUnit,
  ...
  Widget _unitTab() => ListView(padding: const EdgeInsets.all(16), children: [
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 14),
      TextField(controller: _uSizes, decoration: const InputDecoration(labelText: 'Size (jaise: M ya 32) — comma se ek se zyada bhi', hintText: 'M  ya  S,M,L  ya  32', prefixIcon: Icon(Icons.straighten_rounded, color: AppColors.red))),
      const SizedBox(height: 16),
    ...
    ..._units.map((u) {
      ...
      return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 10),
          subs.isEmpty
            ? Text('Koi sub-unit (size) nahi hai abhi.', style: TextStyle(fontSize: 12, color: Colors.grey.shade400))
            : Wrap(spacing: 6, runSpacing: 6, children: subs.map((s) => GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text('${s.size} • ${s.totalQuantity} pcs',
          ...
          if (subs.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 6),
            child: Text('Size pe tap karke low-stock alert limit set karo', style: TextStyle(fontSize: 10, color: Colors.grey.shade400))),
        ]));
    }),
  ]);
  ...
  Widget _itemTab() => ListView(padding: const EdgeInsets.all(16), children: [
    _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 6),
      Text('Unit + Size + Quantity daalo — barcode/naam optional hai.', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
      const SizedBox(height: 16),
      if (_units.isEmpty)
        ...
        DropdownButtonFormField<int>(
          ...
          items: _units.map((u) => DropdownMenuItem(value: u.id, child: Text('${u.name} (${u.gender})'))).toList(),
      ...
      Builder(builder: (_) {
        ...
        if (subs.isEmpty) {
          return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.warning.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)),
            child: const Text('Is unit mein abhi koi size (sub-unit) nahi hai. Pehle "Unit Banao" tab se size add karo.', style: TextStyle(color: AppColors.warning, fontSize: 12)));
        }
        return DropdownButtonFormField<String>(
          value: subs.any((s) => s.size == _selSubUnit) ? _selSubUnit : subs.first.size,
          decoration: const InputDecoration(labelText: 'Sub-Unit (Size) Select Karo', prefixIcon: Icon(Icons.straighten_rounded, color: AppColors.blue)),
          items: subs.map((s) => DropdownMenuItem(value: s.size, child: Text('${s.size} (abhi ${s.totalQuantity} pcs)'))).toList(),
      ...
      const SizedBox(height: 6),
      Text('Optional — barcode wale kapdo ke liye (tracking/scan ke liye)', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
      const SizedBox(height: 10),
      TextField(controller: _iName, decoration: const InputDecoration(labelText: 'Kapde ka naam (optional)', prefixIcon: Icon(Icons.checkroom_rounded, color: AppColors.blue))),
      const SizedBox(height: 12),
    ...
    child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            subUnits.isEmpty
              ? '${unit.gender} • Koi size nahi'
              : '${unit.gender} • Sizes: ${subUnits.map((s) => s.size).join(", ")}',
...
class _UnitItemsScreenState extends State<UnitItemsScreen> {
  ...
  Map<InventorySubUnit, List<InventoryItem>> get _filtered {
    ...
    _grouped.forEach((su, items) {
      ...
      if (matched.isNotEmpty || su.size.toLowerCase().contains(q)) out[su] = matched;
    });
    return out;
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    return Scaffold(
      appBar: GradientAppBar(title: '${widget.unit.name} (${widget.unit.gender})'),
      body: Column(children: [
        Container(color: Colors.white, padding: const EdgeInsets.all(12),
          child: TextField(controller: _search, onChanged: (_) => setState(() {}),
            decoration: InputDecoration(hintText: 'Size, naam ya barcode search karo...', prefixIcon: const Icon(Icons.search_rounded, color: AppColors.blue),
        ...
        Expanded(child: _loading ? buildLoader() : groups.isEmpty
          ? buildEmpty('Is unit mein koi size (sub-unit) nahi hai.', Icons.inventory_2_rounded)
          : ListView(
              ...
              children: groups.entries.map((entry) {
                ...
                return Container(margin: const EdgeInsets.only(bottom: 16),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    // Size ka header — yahi clear karta hai ye kaunsi size ka block hai
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Text('Size: ${su.size}', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: color)),
                        const Spacer(),
                    ...
                    if (items.isEmpty)
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                        child: Text('Koi tracked entry nahi hai is size mein.', style: TextStyle(fontSize: 12, color: Colors.grey.shade400)))
            ...
            : Column(children: (_lines ?? []).map((l) => Container(
                ...
                child: Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('${l.crn.isEmpty ? "No Barcode" : l.crn} • Size: ${l.itemSize}${l.discount > 0 ? " • Disc: ₹${l.discount.toStringAsFixed(0)}" : ""}',
      ...
      Container(color: Colors.white, padding: const EdgeInsets.all(16),
        child: TextField(controller: _ctrl, onChanged: _onType,
          decoration: const InputDecoration(
            hintText: 'Unit search karo (jaise: Jeans, Shirt)...',
                ...
                child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text('${unit.gender} • ${subs.isEmpty ? "Koi size nahi" : "Sizes: ${subs.map((s) => s.size).join(", ")}"}',
                        style: const TextStyle(fontSize: 12, color: AppColors.textGrey)),
                    ])),
                    const Icon(Icons.chevron_right_rounded, color: AppColors.textGrey),
                  ])));
...
class _WorkerUnitPickScreenState extends State<WorkerUnitPickScreen> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final items = await InventoryService.instance.getAllItems(unit: widget.unit.name, gender: widget.unit.gender);
    if (!mounted) return;
              ...
              child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('${item.displayCrn} • ${item.brand} • ${item.size}', style: const TextStyle(fontSize: 12, color: AppColors.textGrey)),
                ...
                child: Padding(padding: const EdgeInsets.all(14), child: Column(children: [
                  ...
                  Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text('${ci.item.displayCrn} \u2022 ${ci.item.brand} \u2022 ${ci.item.size}',
...
class _InventoryAlertScreenState extends State<InventoryAlertScreen> {
  ...
  Future<void> _load() async {
    ...
    for (final r in rows) {
      ...
      final unitLabel = '${r['unit_name']} (${r['unit_gender']})';
      if (su.isOutOfStock) {
      ...
      : (_outMap.isEmpty && _lowMap.isEmpty)
        ...
        : RefreshIndicator(onRefresh: _load, color: AppColors.red,
            child: ListView(padding: const EdgeInsets.all(16), children: [
              ...
              const SizedBox(height: 20),
              Text('Har size (block) ka apna threshold hota hai — "Unit Banao" screen mein size pe tap karke set kar sakte ho.',
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        ...
        child: Row(children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            ...
            child: Text('${subUnits.length} size(s)', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700))),
        ])),
      // Sizes list — ab block (size) level pe dikh raha hai, item level pe nahi
      ...subUnits.map((su) => Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Size: ${su.size}', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
