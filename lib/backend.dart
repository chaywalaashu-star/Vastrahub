...
class AuthService {
  ...
  // ═══════════════════════════════════════════════════════
  // LOGIN CONFIG — Yahan apne credentials rakho
  // ═══════════════════════════════════════════════════════
  ...
  // WORKER keys — jitne workers utni entries add karo
  // Format: 'KEY': 'Worker Name'
  static const Map<String, String> _workerKeys = {
    'WRK-2024-RAM': 'Ramesh Kumar',
    'WRK-2024-SIT': 'Sita Devi',
    'WRK-2024-MOH': 'Mohan Lal',
    // Nayi key add karni ho to yahan likhो:
    // 'WRK-2024-XXX': 'Worker Name',
  };

  // ═══════════════════════════════════════════════════════
  // GOOGLE SHEET LOGIN (baad mein enable karo)
  // NoCode API URL yahan paste karo jab ready ho:
  // static const _sheetUrl = 'https://v2.nocodeapi.com/YOUR/google_sheets/YOUR_KEY';
  // ═══════════════════════════════════════════════════════
  ...
  Future<Result<UserSession>> loginAdmin({
    ...
    // Thoda delay — real feel ke liye
    await Future.delayed(const Duration(milliseconds: 600));

    // Username aur password check karo
    if (username.trim() == _adminUsername &&
      ...
      debugPrint('[Auth] Admin login success');
  ...
  Future<Result<UserSession>> loginWorker({required String workerKey}) async {
    ...
    // Key list mein check karo
    if (_workerKeys.containsKey(key)) {
      final workerName = _workerKeys[key]!;
      final session = UserSession(
        userId: 'worker_${key.replaceAll('-', '_').toLowerCase()}',
        name: workerName,
        role: 'worker',
        token: 'local_token_${DateTime.now().millisecondsSinceEpoch}',
        workerKey: key,
      ...
      await _saveSession(session);
      debugPrint('[Auth] Worker login: $workerName');
      return Result.success(session);
    }
