...
class AuthService {
  ...
  Future<List<Map<String, dynamic>>> _fetchUsers() async {
    try {
      debugPrint('[Auth] Fetching users from sheet...');

      // Google Apps Script redirect handle karta hai
      final client = http.Client();
      var req = http.Request('GET', Uri.parse(AppConfig.scriptUrl));
      ...
      debugPrint('[Auth] Body: ${res.body.substring(0, res.body.length.clamp(0, 300))}');
  ...
  Future<Result<UserSession>> loginAdmin({
    ...
    try {
      ...
      return const Result.failure('Internet nahi hai. WiFi/Data on karo.');
    } catch (e) {
  ...
  Future<Result<void>> createAccount({
    ...
    try {
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        debugPrint('[Auth] Sheet error: ${res.statusCode} — ${res.body}');
