...
class AuthService {
  ...
  /// Admin login — server se verify karta hai
  /// Returns: Result<UserSession>
  Future<Result<UserSession>> loginAdmin({
    required String username,
    required String password,
  }) async {
    // Internet check karo — admin login ke liye zaroori hai
    final online = await ConnectivityService.instance.hasRealInternet;
    if (!online) {
      return const Result.failure(
          'Admin login ke liye internet zaroori hai. Kripya internet connect karein.');
    }

    try {
      final response = await http
          .post(
            Uri.parse('${AppConfig.apiBaseUrl}/admin/login'),
      ...
      if (response.statusCode == 200) {
        ...
        return Result.failure(
            'Server error: ${response.statusCode}. Baad mein try karein.');
      }
    } on SocketException {
      return const Result.failure('Internet connection nahi mili. Check karein.');
    } on HttpException {
      return const Result.failure('Server se connect nahi ho paya.');
    } catch (e) {
      return Result.failure('Login error: $e');
    }
  }

  /// Worker login — worker key se (online verify karta hai)
  Future<Result<UserSession>> loginWorker({required String workerKey}) async {
    final online = await ConnectivityService.instance.hasRealInternet;
    if (!online) {
      return const Result.failure(
          'Worker login ke liye internet zaroori hai. Kripya internet connect karein.');
    }

    try {
      final response = await http
          .post(
            Uri.parse('${AppConfig.apiBaseUrl}/worker/login'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({'worker_key': workerKey}),
          )
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body) as Map<String, dynamic>;
        final session = UserSession(
          userId: data['worker_id'] as String,
          name: data['name'] as String,
          role: 'worker',
          token: data['token'] as String,
          workerKey: workerKey,
        ...
        // Worker login ke baad latest inventory download karo
        await InventoryService.instance.downloadInventoryFromServer(session.token);

        return Result.success(session);
      } else if (response.statusCode == 401) {
        return const Result.failure(
            'Worker key invalid ya inactive hai. Admin se contact karein.');
      } else {
        return Result.failure('Server error: ${response.statusCode}');
      }
    } on SocketException {
      return const Result.failure('Internet connection nahi hai.');
    } catch (e) {
      return Result.failure('Login error: $e');
    }
  }

  /// Logout — pehle sync karta hai, phir logout
  /// Worker ke liye: net chahiye, sync hogi phir local data delete hoga
  /// Admin ke liye: directly logout
  Future<Result<void>> logout() async {
    ...
    if (session.isWorker) {
      ...
      if (!online) {
        return const Result.failure(
            'Logout ke liye internet zaroori hai taaki aapka data server pe save ho sake. '
            'Internet connect karein aur phir try karein.');
      }

      // Unsync data sync karo
      final syncResult = await SyncService.instance.syncWorkerData(session);
      if (!syncResult.isSuccess) {
        return Result.failure(
            'Data sync failed: ${syncResult.error}. Logout nahi ho sakta.');
      }

      // Sync hone ke baad worker ka local sales/returns data delete karo
      await SyncService.instance.clearSyncedWorkerData(session.userId);
    }
...
class InventoryService {
  ...
  /// Quantity update karo (sell ya return ke baad)
  Future<Result<void>> _updateQuantity(String crn, int delta) async {
    // delta: negative = sell, positive = return
    final item = await getItemByCrn(crn);
    if (item == null) return const Result.failure('Item nahi mila.');

    final newQty = item.quantity + delta;
    if (newQty < 0) {
      return const Result.failure('Itni quantity stock mein nahi hai.');
    }
