...
class InventoryItem {
  ...
  final String size;
  final String unit;
  final int? subUnitId;    // Kis block (Unit+Size) ka hissa hai — stock yahi se judi hai
  final String brand;
  final double price;
  ...
  // Naam nahi diya to Unit+Size se ek readable naam ban jata hai
  String get displayName => name.trim().isEmpty ? '$unit $size' : name;
...
class InventorySubUnit {
  ...
  final int unitId;
  final String size;
  int totalQuantity; // ASLI stock count is size (block) ke liye
...
class DatabaseService {
    ...
    return await openDatabase(
      ...
      onUpgrade: (db, oldV, newV) async {
        ...
        if (oldV < 4) {
          ...
          // ── 1. Units: naam+gender dono se UNIQUE (pehle sirf naam se tha,
          //    isliye "Jeans" Men aur "Jeans" Women ek hi unit ban jaate the) ──
          await db.execute('''
          ...
          ''');
          await db.execute('''
            INSERT OR IGNORE INTO inventory_units_new (id, name, gender)
            SELECT id, name, COALESCE(gender, 'Men') FROM inventory_units
          ''');
          ...
          await db.execute('''
            INSERT INTO inventory_new
              (id, crn, name, gender, size, unit, sub_unit_id, brand, price, quantity, created_at, updated_at)
            SELECT
              i.id, i.crn, i.name, i.gender, i.size, i.unit,
              (SELECT su.id FROM inventory_sub_units su
                 JOIN inventory_units iu ON iu.id = su.unit_id
                 WHERE LOWER(iu.name) = LOWER(i.unit) AND LOWER(iu.gender) = LOWER(i.gender)
                   AND LOWER(su.size) = LOWER(i.size)
                 LIMIT 1),
          ...
          debugPrint('[DB] Migrated to version 4 (block-based stock + sale sessions + unit gender fix)');
        }
      },
  ...
  Future<void> _createTables(Database db, int version) async {
    // Inventory Units table — naam+gender dono se unique (Jeans-Men aur
    // Jeans-Women alag alag units hain)
    await db.execute('''
    ...
    // se pata chalta hai ye item kis block (size) ka hissa hai.
    await db.execute('''
...
class InventoryService {
  ...
  /// Unit banao (ya agar naam+gender se already exist karti hai to wahi use
  /// karo), aur diye gaye size(s) ko us unit ke SUB-UNIT ke roop mein add
  /// karo. Isse same naam ki unit dobara banane par purani REPLACE nahi hoti
  /// — naya size sirf ek nayi sub-unit ban kar existing unit ke andar jud
  /// jaata hai. Naam+Gender dono match karne par hi existing unit maani
  /// jaati hai — isliye "Jeans" Men aur "Jeans" Women alag-alag units hain.
  ///
  ...
  Future<Result<InventoryUnit>> addOrUpdateUnit({
    ...
    try {
      ...
      // Existing unit dhundho — NAAM aur GENDER dono match hone chahiye
      final existing = await db.query('inventory_units',
          where: 'LOWER(name) = ? AND LOWER(gender) = ?',
          whereArgs: [cleanName.toLowerCase(), gender.toLowerCase()]);
      ...
      debugPrint('[Inventory] Unit "$cleanName" ($gender) ready with sizes: ${sizeList.join(", ")}');
  ...
  /// na diya ho, quantity hamesha us Unit+Size ke SUB-UNIT (block) ke total
  ...
  Future<Result<InventoryItem>> addItem(InventoryItem item, {bool silent = false}) async {
    try {
      ...
      // Is Unit+Gender+Size ka block (sub-unit) dhundo
      final subRows = await db.rawQuery('''
        SELECT su.* FROM inventory_sub_units su
        JOIN inventory_units iu ON iu.id = su.unit_id
        WHERE LOWER(iu.name) = ? AND LOWER(iu.gender) = ? AND LOWER(su.size) = ?
        LIMIT 1
      ''', [item.unit.trim().toLowerCase(), item.gender.trim().toLowerCase(), item.size.trim().toLowerCase()]);

      if (subRows.isEmpty) {
        return const Result.failure(
            'Ye size is unit mein nahi mili. Pehle "Unit Banao" tab se ye size add karo.');
      }
      ...
      if (crn.isNotEmpty) {
        ...
        if (dup.isNotEmpty) {
          ...
          itemId = await db.insert('inventory', InventoryItem(
            crn: crn, name: item.name.trim(), gender: item.gender, size: item.size,
        ...
        itemId = await db.insert('inventory', InventoryItem(
          crn: '', name: item.name.trim(), gender: item.gender, size: item.size,
          unit: item.unit, subUnitId: subUnit.id, brand: item.brand,
          price: item.price, quantity: addQty, createdAt: now, updatedAt: now,
        ).toMap());
      }
      ...
      debugPrint('[Inventory] Stock added: ${item.unit}/${item.size} +$addQty (${saved.displayCrn})');
  ...
  /// UnitItemsScreen mein "Size: M — Total: 12" jaisi listing ke liye
  ...
  /// alag threshold hota hai, aur check Unit+Size ke TOTAL par hota hai,
  /// na ki har item ki apni alag quantity par (jo pehle galat tha).
  Future<List<Map<String, dynamic>>> getLowStockSubUnits() async {
    final db = await _db.database;
    final rows = await db.rawQuery('''
      SELECT su.*, iu.name AS unit_name, iu.gender AS unit_gender
      ...
      await db.transaction((txn) async {
        ...
        for (int i = 0; i < resolvedItems.length; i++) {
          ...
          savedLines.add(SaleRecord(
            ...
            itemGender: sale.itemGender, itemSize: sale.itemSize, brand: sale.brand,
  ...
  Future<Result<int>> importFromExcel() async {
    try {
      ...
      for (int i = 1; i < sheet.rows.length; i++) {
        ...
        // Sirf Unit + Size zaroori hain — CRN/naam na ho tab bhi stock
      ...
      if (addedCount == 0) {
        return Result.failure(
            ...
            '• Columns: Unit, Sub Unit (Size), Item Name, CRN, Gender, Brand, Price, Qty\n'
