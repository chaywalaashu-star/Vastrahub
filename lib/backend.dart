...
class AuthService {
  ...
  Future<List<Map<String, dynamic>>> _fetchUsers() async {
    try {
      debugPrint('[Auth] Fetching users...');

      // Simple GET — redirects auto follow hote hain
      final res = await http
          .get(Uri.parse(AppConfig.scriptUrl))
          .timeout(const Duration(seconds: 20));

      debugPrint('[Auth] Status: ${res.statusCode}');
      debugPrint('[Auth] Body preview: ${res.body.substring(0, res.body.length.clamp(0, 200))}');
      ...
      debugPrint('[Auth] No internet connection');
    } on TimeoutException {
