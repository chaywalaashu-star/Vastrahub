...
class AuthService {
  ...
  Future<Result<UserSession>> loginWorker({required String workerKey}) async {
    try {
      ...
      final match = users.where((u) {
        return (u['worker_key'] ?? '').toString().trim().toUpperCase() == key &&
            (u['role']     ?? '').toString().trim().toLowerCase() == 'worker' &&
            (u['is_active']?? 'true').toString().trim().toLowerCase() == 'true';
      }).toList();

      if (match.isEmpty) {
        return const Result.failure(
            'Worker key galat hai ya inactive.\nAdmin se contact karein.');
      }

      final session = UserSession(
        userId: 'worker_${match.first['row_id'] ?? 1}',
        name: (match.first['name'] ?? 'Worker').toString(),
        role: 'worker',
        token: 'token_${DateTime.now().millisecondsSinceEpoch}',
        workerKey: key,
      );
  ...
  Future<Result<int>> importFromExcel() async {
    try {
      ...
      for (int i = 1; i < sheet.rows.length; i++) {
        ...
        // Safe value nikalo
        String safeVal(int col, String fallback) {
          try {
        ...
        try {
          final item = InventoryItem(
            crn:      crn.toUpperCase(),
            name:     name,
            gender:   safeVal(2, 'Men'),
            size:     safeVal(3, 'Free Size'),
            unit:     safeVal(4, 'Piece'),
            brand:    safeVal(5, '-'),
            price:    double.tryParse(safeVal(6, '0')) ?? 0,
            quantity: int.tryParse(safeVal(7, '0')) ?? 0,
          );

          final r = await InventoryService.instance.addItem(item);
      ...
      if (addedCount == 0) {
        return Result.failure(
            ...
            '• Columns: CRN, Name, Gender, Size, Unit, Brand, Price, Qty');
      }
